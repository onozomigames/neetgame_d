...
namespace Utage
{
	...
	public class SampleUiTipsMasterDetail : UguiView
	{
		...
		public List<TipsCategoryInfo> TipsCategories { get; } = new();
		...
		void InitCategories()
		{
			...
			foreach (var keyValuePair in TipsManager.TipsMap)
			{
				...
				var categoryInfo = TipsCategories.Find(x => x.Id == category);
				if (categoryInfo == null)
				{
					categoryInfo = new TipsCategoryInfo { Id = category };
					TipsCategories.Add(categoryInfo);
				}
		...
		protected virtual void InitCategoryButtons()
		{
			...
			{
				if (!buttons.Exists(x => x.CategoryInfo == category))
				{
					//カテゴリボタンが見つからない
					Debug.LogError($" CategoryButton is not found categoryId={category.Id}");
				}
			}
		...
		void InitSelected()
		{
			...
			if (reserved)
			{
				...
				for (int i = 0; i < TipsCategories.Count; i++)
				{
					var categoryInfo = TipsCategories[i];
			...
			else
			{
				indexCategory = 0;
				indexTips = 0;
				TipsInfo = TipsCategories[indexCategory].TipsList[indexTips];
			}
			...
			CreateTipsButtons(TipsCategories[indexCategory]);
		...
		void OnChangedCategory(int index)
		{
			...
			var categoryInfo = TipsCategories[index];
		...
		{
			if(!IsInit) return;
			var categoryInfo = TipsCategories[categoryToggleGroup.CurrentIndex];
